...
void encoders_hctl_init(void)
{
  ...
  // Initialisation
   ...
   DDRF = B00000000;   // sets Arduino Mega (ATMEL ATMEGA) Digital pins 22(PORTA0) to 29(PORTA7) as inputs from HCTL-2032 - D0 to D7
...
if (encoder==1)
   digitalWrite(XY,   HIGH); // Selection de l'encodeur 1 ou 2 (ici : 1)
   ...
   IRQ_LOCK(flags);
   g_encoders_hctl_values[0] = count;
   IRQ_UNLOCK(flags);

    
   return g_encoders_hctl_values[0];
}
